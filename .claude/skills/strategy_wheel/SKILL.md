---
name: strategy_wheel
description: This skill should be used when invoked by master_trading with the wheel strategy enabled. The wheel: sell cash-secured puts → if assigned, hold the underlying and sell covered calls → if called away, restart by selling puts again. Currently disabled by default because it requires Alpaca options account approval; the script short-circuits with a no-op until enabled in strategy_defaults.json.
version: 0.1.0
---
